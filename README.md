...
3. When the user provides a scenario PDF or folder plus a scenario name, stage the local source text with the import script, then author `scenarios/<scenario-name>/npcs-and-enemies.md`, `scenarios/<scenario-name>/story-overview.md`, and `scenarios/<scenario-name>/keeper-notes.md` as the final keeper package in the source document language before building derivative keeper notes. `keeper-notes.md` may reference `npcs-and-enemies.md`, but the NPC file should remain standalone. Finish with an explicit validation pass against `.source-extract.md` so essential details are not lost in summarization.
