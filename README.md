...
- `.agents/skills/`: canonical shared on-demand workflows and slash-command skills for both GitHub Copilot and Claude Code (single source of truth; no `.github/skills` or `.claude/skills` wrappers).
...
Shared operating rules live in `.agents/assistant-spec.md`. Keep `.github/` and `.claude/` limited to agent-specific files.
...
- `.agents/skills/import-scenario/`: shared skill for turning a local scenario PDF or PDF folder into a standardized keeper-facing scenario package under `scenarios/`.
...
- `uv run python .agents/scripts/import_scenario.py <pdf-or-folder> <scenario-name>` builds internal staging material in `scenarios/<scenario-name>/.source-extract.md` for the assistant-authored final scenario package.
...
3. When the user provides a scenario PDF or folder plus a scenario name, stage the local source text with the import script so that `.source-extract.md` already contains only the scoped scenario material, then author `scenarios/<scenario-name>/npcs-and-enemies.md`, `scenarios/<scenario-name>/story-overview.md`, `scenarios/<scenario-name>/keeper-notes.md`, `scenarios/<scenario-name>/clue-map.md`, and `scenarios/<scenario-name>/session-plan.md` as the final keeper package in the source document language before building derivative keeper notes. `keeper-notes.md` may reference `npcs-and-enemies.md`, but the NPC file should remain standalone. Finish with an explicit validation pass against the scoped scenario material in `.source-extract.md` so essential details are not lost in summarization and details from neighboring scenarios do not leak in.
